...
void Weapon::shoot()
{
    if(owner) {
        if(canFire && !canSpread) {
            ...
            playerBullet->setPos(owner->x() + owner->getWidth()/5,owner->y()-owner->getHeight()/8 - owner->getHeight()/7);
        }
        ...
        if(canFire && canSpread) {
            ...
            if(numberOfPellets % 2 == 0) {
                for(int i = 1; i <= middle; ++i) {
                    ...
                    playerBullet->setPos(owner->x()+ owner->getWidth()/5,owner->y()-owner->getHeight()/8 - owner->getHeight()/7);
                    playerBulletComp->setPos(owner->x()+ owner->getWidth()/5,owner->y()-owner->getHeight()/8 - owner->getHeight()/7);
            ...
            //Odd Number Of Pellets
                ...
                for(int i = 1; i <= middle; ++i) {
                    ...
                    if(i < middle) {
                        ...
                        owner->scene()->addItem(playerBullet);
                        owner->scene()->addItem(playerBulletComp);
                        playerBullet->setPos(owner->x(),owner->y()-owner->getHeight()/8 - owner->getHeight()/7);
                        playerBulletComp->setPos(owner->x(),owner->y()-owner->getHeight()/8 - owner->getHeight()/7);
                    }
                    if (i == middle) {
                        //x-Direction function of number of pellet
                        //Spreading weapons are faster than normal
